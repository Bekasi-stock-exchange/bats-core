...
func (r *Master) LoadEmiten(ctx context.Context) ([]market.Emiten, error) {
	return postgres.QueryAll(ctx, r.pool, "emiten",
		...
		func(rows pgx.Rows) (market.Emiten, error) {
			var e market.Emiten
			err := rows.Scan(&e.ID, &e.Kode, &e.Nama, &e.ListedShares, &e.UnlistedShares, &e.IsActive)
			return e, err
		})
}
...
func (r *Master) CreateEmiten(ctx context.Context, e market.Emiten) (market.Emiten, error) {
	err := r.pool.QueryRow(ctx, `
		INSERT INTO emiten (kode, nama, listed_shares, unlisted_shares, is_active)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id`,
		e.Kode, e.Nama, e.ListedShares, e.UnlistedShares, e.IsActive,
	).Scan(&e.ID)
	if err != nil {
